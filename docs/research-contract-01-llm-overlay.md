...
- each classified failed attempt persists one `r01-acquisition-failure-v3` record. When a process capture exists, that record binds the command spec, exact stdout JSONL, stderr, process status, and any parsed provider-response artifact through `r01-codex-attempt-failure-transport-artifacts-v1`. A later successful attempt binds every prior retry record into the run result. Historical v2 records remain parseable but cannot contain post-response stop artifacts;
...
- if a complete response exceeds the approved per-attempt token reserve, the runner persists `r01-acquisition-failure-v3` before raising `STOP_PHASE`. Its `r01-codex-post-response-stop-artifacts-v1` edge binds the request artifacts, raw response, provider-response metadata, token usage, and any complete transport graph. The over-reserve response is never scored and cannot enter a run result;
...
- command-spec validation and raw/provider-response artifact persistence failures are converted to explicit `CodexExecError(STOP_PHASE)` records rather than escaping the acquisition audit boundary. Failure to persist the raw transport before parsing is non-retryable and stops the acquisition path.
