...
- the complete B2 report is `docs/r01-b2-zero-call-preflight.md`, SHA-256
  `9695aac7d3fb77c2a9d0f54f19ed8ca6a73601513a709838d2f1ca8072c1ce8c`,
...
- each acquisition binds an `r01-complete-response-disposition-v1` artifact and
  the containing result schema is bumped to `r01-development-run-result-v4`;
- updated research contract SHA-256:
  `f1e46e11397ab1dd41cc1bda3c2f2c20720201733bb2d2e6a192d6cf241c69f3`;
- `121` provider-free overlay tests pass, including dedicated consumer,
  one-hold, bounded-retry, first-nonzero stop, global-attempt-cap,
  missing-consumer, stop-before-score, raw-text binding, failure-transport
  replay, command-build/persistence classification, anchor, and token-ledger
  coverage;
- D2 recommendations and approved resolution are recorded in
  `docs/r01-d2-decision-package.md`, SHA-256
  `bf9597c14460fdebf104fd7ff10789256857f07a3a2d44c086d0afa527099d8e`;
...
The zero-cost account condition is attested in
`docs/r01-b3-zero-cost-account-attestation.md`, SHA-256
`94ca690340273e02da7de19e0c1ea5efc8793547f2a87822dc40eb5b630b9746`:
personal Pro, purchased-credit balance `KRW 0`, Auto top-up closed without
saving, and no selected shared-credit workspace. The first B3 call remains
blocked until the live B3 command, deterministic six-case anchor manifest,
command specs, and reservation/actual-token ledger are generated by the
provider-free preflight, hashed, reviewed, and externally frozen. D2 approval
does not waive these operational requirements.
...
#### B3 failure-audit cross-review closure — 2026-07-16

- command-spec validation failures are classified as `STOP_PHASE` before a
  provider call and receive an acquisition-failure record;
- raw-capture and parsed-provider artifact collisions/integrity failures are
  classified as `STOP_PHASE`, not untyped crashes;
- each retry failure binds its command spec, stdout JSONL, stderr, process
  status, optional parsed response, and token reservation through
  `r01-acquisition-failure-v2`;
- replay verifies every failed-attempt byte/hash edge before accepting a later
  successful acquisition;
- retry identity changes use full Pydantic revalidation, so attempt 3 is blocked
...
The first acquisition of the reviewed 12-call B3 plan is the live smoke
boundary; it is not an additional thirteenth call. The 12-call command remains
blocked until its code commit and one-shot preflight hash pass external review.
After B3, verify artifacts, token usage, tool prohibition, replay, and budget
accounting before authorizing B4 or any further provider acquisition.
